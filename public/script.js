...
const RAD_STUDIO_CONV = {
    id: "conv_rad_studio_recursividad",
    title: "Plantilla RAD Studio C++: Recursividad",
    interactionId: "rad_studio_template",
    messages: [
        {
            role: "user",
            text: "Extrae el texto de las imágenes de clase y genera la plantilla funcional para resolver ejercicios de recursividad en C++ con RAD Studio (VCL, formularios, botones, vectores, matrices y cadenas).",
            ts: 1789819200000
        },
        {
            role: "model",
            text: "========================================================================\nGUIA COMPLETA: APLICACION VCL EN C++BUILDER (RECURSIVIDAD PURA)\n========================================================================\n\nEsta guia contiene todos los pasos para crear tu aplicacion sin usar \nningun bucle (for, while, do-while), utilizando unicamente recursividad.\n\nPASO 1: CREAR EL PROYECTO\n------------------------------------------------------------------------\n1. Abre RAD Studio.\n2. Ve al menu \"File\" > \"New\" > \"Windows VCL Application - C++Builder\".\n3. Guarda inmediatamente tu proyecto en una carpeta nueva usando \n   \"File\" > \"Save All\". Al pedir guardar la Unidad, guardala como Unit1.cpp,\n   y el Proyecto como Project1.cbproj.\n\nPASO 2: CREAR EL DISENO VISUAL\n------------------------------------------------------------------------\nArrastra los siguientes componentes desde la Tool Palette a tu formulario \ny cambia sus propiedades Name y Caption en el Object Inspector:\n\n- Componente: TStringGrid\n  Name: StringGridVector\n  Caption: (Vacio)\n\n- Componente: TStringGrid\n  Name: StringGridMatriz\n  Caption: (Vacio)\n\n- Componente: TButton\n  Name: btnLlenarVector\n  Caption: Llenar Vector\n\n- Componente: TButton\n  Name: btnEj1\n  Caption: 1. Ordenar Digitos\n\n- Componente: TButton\n  Name: btnEj2\n  Caption: 2. Verificar Palindromo\n\n- Componente: TButton\n  Name: btnEj3\n  Caption: 3. Cargar Matriz (Vaiven / Serpiente)\n\n- Componente: TButton\n  Name: btnEj4\n  Caption: 4. Frecuencia Digitos\n\n- Componente: TButton\n  Name: btnEj5\n  Caption: 5. Eliminar Vocal-Consonante\n\n* IMPORTANTE: Solo acomoda los componentes en el formulario. No hagas \ndoble clic en los botones todavia.\n\n\nPASO 3: ANADIR EL CODIGO (RECURSIVIDAD PURA)\n------------------------------------------------------------------------\n\n3.1. Editar el Archivo de Cabecera (Unit1.h)\nAbre el archivo Unit1.h en RAD Studio. Busca la seccion correspondiente y \npega el siguiente codigo, asegurandote de que quede asi:\n\n```cpp\n//---------------------------------------------------------------------------\n#ifndef Unit1H\n#define Unit1H\n//---------------------------------------------------------------------------\n#include <System.Classes.hpp>\n#include <Vcl.Controls.hpp>\n#include <Vcl.StdCtrls.hpp>\n#include <Vcl.Forms.hpp>\n#include <Vcl.Grids.hpp>\n//---------------------------------------------------------------------------\n\nclass TForm1 : public TForm\n{\n__published:\t// IDE-managed Components\n\tTStringGrid *StringGridVector;\n\tTStringGrid *StringGridMatriz;\n\tTButton *btnLlenarVector;\n\tTButton *btnEj1;\n\tTButton *btnEj2;\n\tTButton *btnEj3;\n\tTButton *btnEj4;\n\tTButton *btnEj5;\n\n\t// Eventos de los Botones\n\tvoid __fastcall btnLlenarVectorClick(TObject *Sender);\n\tvoid __fastcall btnEj1Click(TObject *Sender);\n\tvoid __fastcall btnEj2Click(TObject *Sender);\n\tvoid __fastcall btnEj3Click(TObject *Sender);\n\tvoid __fastcall btnEj4Click(TObject *Sender);\n\tvoid __fastcall btnEj5Click(TObject *Sender);\n\nprivate:\t// User declarations\n\t// --- Funciones de Logica Recursiva Pura (Cero bucles for/while) ---\n\tvoid InsertarEnOrden(TStringGrid *v, byte &n, byte a);\n\tvoid OrdenarDigitos(Cardinal x, TStringGrid *v, byte &n);\n\tbool EsPalindromo(TStringGrid *v, byte a, byte b);\n\tvoid LlenarFila(TStringGrid *v, int fila, int colInicio, int colFin, int &k);\n\tvoid CargarMatrizRecursivo(TStringGrid *v, int fila, int colInicio, int colFin, int &k);\n\tvoid LlenarCeros(TStringGrid *v, byte n);\n\tvoid CargarVecFrecuencia(TStringGrid *v, byte n, Cardinal x);\n\tbool EsVoc(char x);\n\tbool EsCons(char x);\n\tvoid ElimStr(AnsiString &x);\n\n\t// --- Funciones Auxiliares para evitar bucles (Iteracion abstraida) ---\n\tvoid LlenarVectorRecursivo(TStringGrid *v, int i, int n);\n\npublic:\t\t// User declarations\n\t__fastcall TForm1(TComponent* Owner);\n};\n//---------------------------------------------------------------------------\nextern PACKAGE TForm1 *Form1;\n//---------------------------------------------------------------------------\n#endif\n```\n\n\n3.2. Reemplazar el Codigo del Archivo Fuente (Unit1.cpp)\nAbre el archivo Unit1.cpp. Selecciona TODO el texto (Ctrl+A), borralo \npor completo y pega el siguiente codigo exactamente como esta:\n\n```cpp\n//---------------------------------------------------------------------------\n#include <vcl.h>\n#pragma hdrstop\n\n#include \"Unit1.h\"\n//---------------------------------------------------------------------------\n#pragma package(smart_init)\n#pragma resource \"*.dfm\"\nTForm1 *Form1;\n//---------------------------------------------------------------------------\n__fastcall TForm1::TForm1(TComponent* Owner)\n\t: TForm(Owner)\n{\n\tStringGridVector->RowCount = 1;\n\tStringGridVector->ColCount = 1;\n\tStringGridVector->Cells[0][0] = \"\";\n\tStringGridMatriz->RowCount = 1;\n\tStringGridMatriz->ColCount = 1;\n\tStringGridMatriz->Cells[0][0] = \"\";\n}\n//---------------------------------------------------------------------------\n\n// --- EJERCICIO 1: ORDENAR DIGITOS EN VECTOR (RECURSIVIDAD PURA) ---\nvoid TForm1::InsertarEnOrden(TStringGrid *v, byte &n, byte a) {\n\tif (n == 0) { \n\t\tn++; \n\t\tv->Cells[0][0] = a; \n\t} else {\n\t\tbyte e = StrToInt(v->Cells[n-1][0]);\n\t\tif (a < e) { \n\t\t\tn--; \n\t\t\tInsertarEnOrden(v, n, a); \n\t\t\tv->Cells[n][0] = e; \n\t\t\tn++; \n\t\t} else { \n\t\t\tv->Cells[n][0] = a; \n\t\t\tn++; \n\t\t}\n\t}\n}\n\nvoid TForm1::OrdenarDigitos(Cardinal x, TStringGrid *v, byte &n) {\n\tif (x > 0) { \n\t\tbyte a = x % 10; \n\t\tOrdenarDigitos(x / 10, v, n); \n\t\tInsertarEnOrden(v, n, a); \n\t}\n}\n\n// --- EJERCICIO 2: VERIFICAR PALINDROMO EN VECTOR (RECURSIVIDAD PURA) ---\nbool TForm1::EsPalindromo(TStringGrid *v, byte a, byte b) {\n\t// Caso base: Si los indices se cruzan o son iguales, es palindromo\n\tif (a >= b) return true; \n\t\n\t// Cortocircuito logico: Si los extremos son distintos, corta la recursion\n\tif (v->Cells[a][0] != v->Cells[b][0]) return false; \n\t\n\t// Hipotesis: Asume que el resto del vector se verifica correctamente\n\treturn EsPalindromo(v, a + 1, b - 1);\n}\n\n// --- EJERCICIO 3: CARGAR MATRIZ EN SERPIENTE / VAIVEN (RECURSIVIDAD PURA) ---\nvoid TForm1::LlenarFila(TStringGrid *v, int fila, int colInicio, int colFin, int &k) {\n\tif (colInicio > colFin) return; \n\t\n\tif ((v->RowCount - 1 - fila) % 2 != 0) { \n\t\tv->Cells[colInicio][fila] = k++; \n\t\tLlenarFila(v, fila, colInicio + 1, colFin, k); \n\t} else {\n\t\tv->Cells[colFin][fila] = k++; \n\t\tLlenarFila(v, fila, colInicio, colFin - 1, k); \n\t}\n}\n\nvoid TForm1::CargarMatrizRecursivo(TStringGrid *v, int fila, int colInicio, int colFin, int &k) {\n\tif (fila < 0) return; \n\t\n\tLlenarFila(v, fila, colInicio, colFin, k); \n\tCargarMatrizRecursivo(v, fila - 1, colInicio, colFin, k); \n}\n\n// --- EJERCICIO 4: FRECUENCIA DE DIGITOS EN VECTOR (PIZARRA WHATSAPP) ---\nvoid TForm1::LlenarCeros(TStringGrid *v, byte n) {\n\tif (n > 0) {\n\t\tLlenarCeros(v, n - 1);\n\t\tv->Cells[n - 1][0] = \"0\";\n\t}\n}\n\nvoid TForm1::CargarVecFrecuencia(TStringGrid *v, byte n, Cardinal x) {\n\tif (x > 0) {\n\t\tbyte z = x % 10;\n\t\tCargarVecFrecuencia(v, n, x / 10);\n\t\tif (z < n) {\n\t\t\tint actual = StrToIntDef(v->Cells[z][0], 0);\n\t\t\tv->Cells[z][0] = IntToStr(actual + 1);\n\t\t}\n\t} else {\n\t\tLlenarCeros(v, n);\n\t}\n}\n\n// --- EJERCICIO 5: ELIMINAR VOCAL SEGUIDA DE CONSONANTE (PIZARRA WHATSAPP) ---\nbool TForm1::EsVoc(char x) {\n\tAnsiString Voc = \"AEIOUaeiou\";\n\treturn (Voc.Pos(x) > 0);\n}\n\nbool TForm1::EsCons(char x) {\n\tbool esLetra = ((x >= 'a' && x <= 'z') || (x >= 'A' && x <= 'Z'));\n\treturn esLetra && !EsVoc(x);\n}\n\nvoid TForm1::ElimStr(AnsiString &x) {\n\tAnsiString z;\n\tbyte m = x.Length();\n\tif (m > 1) {\n\t\tif (EsVoc(x[1]) && EsCons(x[2])) {\n\t\t\tz = \"\";       // Se descarta la vocal\n\t\t} else {\n\t\t\tz = x[1];     // Se conserva el caracter\n\t\t}\n\t\tx.Delete(1, 1);\n\t\tElimStr(x);\n\t\tx = z + x;        // Reconstruccion al desapilar\n\t}\n}\n\n// --- FUNCIONES AUXILIARES: ENTRADA RECURSIVA SIN BUCLES ---\nvoid TForm1::LlenarVectorRecursivo(TStringGrid *v, int i, int n) {\n\tif (i >= n) return; \n\t\n\tString dato = InputBox(\"Llenar Vector\", \"Ingrese el elemento para la posicion \" + IntToStr(i + 1) + \":\", \"\");\n\tv->Cells[i][0] = dato;\n\tLlenarVectorRecursivo(v, i + 1, n); \n}\n\n// --- EVENTOS DE LOS BOTONES ---\n\nvoid __fastcall TForm1::btnLlenarVectorClick(TObject *Sender)\n{\n\tint n = StrToInt(InputBox(\"Llenar Vector\", \"Ingrese la cantidad de elementos:\", \"5\"));\n\tif (n <= 0) return;\n\t\n\tStringGridVector->ColCount = n;\n\tLlenarVectorRecursivo(StringGridVector, 0, n); \n\tShowMessage(\"Vector llenado correctamente mediante recursion pura.\");\n}\n\nvoid __fastcall TForm1::btnEj1Click(TObject *Sender)\n{\n\tCardinal x = StrToInt(InputBox(\"Ejercicio 1: Ordenar Digitos\", \"Ingrese el numero a ordenar:\", \"37152\"));\n\tif (x == 0) {\n\t\tStringGridVector->ColCount = 1;\n\t\tStringGridVector->Cells[0][0] = \"0\";\n\t\treturn;\n\t}\n\t\n\tbyte n = 0;\n\tOrdenarDigitos(x, StringGridVector, n);\n\tStringGridVector->ColCount = n;\n\tShowMessage(\"Los digitos ordenados se muestran en el vector.\");\n}\n\nvoid __fastcall TForm1::btnEj2Click(TObject *Sender)\n{\n\tif (StringGridVector->ColCount <= 1 && StringGridVector->Cells[0][0] == \"\") {\n\t\tShowMessage(\"El vector esta vacio. Use 'Llenar Vector' primero.\"); \n\t\treturn;\n\t}\n\t\n\tbool resultado = EsPalindromo(StringGridVector, 0, StringGridVector->ColCount - 1);\n\tif (resultado) \n\t\tShowMessage(\"Resultado: El vector ES palindromico (Verdadero).\");\n\telse \n\t\tShowMessage(\"Resultado: El vector NO es palindromico (Falso).\");\n}\n\nvoid __fastcall TForm1::btnEj3Click(TObject *Sender)\n{\n\tint M = StrToInt(InputBox(\"Ejercicio 3: Cargar Matriz\", \"Cantidad de Filas (M):\", \"5\"));\n\tint N = StrToInt(InputBox(\"Ejercicio 3: Cargar Matriz\", \"Cantidad de Columnas (N):\", \"4\"));\n\t\n\tStringGridMatriz->RowCount = M; \n\tStringGridMatriz->ColCount = N;\n\tint k = 1;\n\t\n\tCargarMatrizRecursivo(StringGridMatriz, M - 1, 0, N - 1, k); \n\tShowMessage(\"Matriz cargada segun el patron de vaiven recursivo.\");\n}\n\nvoid __fastcall TForm1::btnEj4Click(TObject *Sender)\n{\n\tCardinal x = StrToInt(InputBox(\"Ejercicio 4: Frecuencia de Digitos\", \"Ingrese el numero (x):\", \"1254321\"));\n\tStringGridVector->ColCount = 10;\n\tCargarVecFrecuencia(StringGridVector, 10, x);\n\tShowMessage(\"Frecuencias calculadas en StringGridVector (posiciones 0 a 9).\");\n}\n\nvoid __fastcall TForm1::btnEj5Click(TObject *Sender)\n{\n\tAnsiString texto = InputBox(\"Ejercicio 5: Eliminar Vocal-Consonante\", \"Ingrese la cadena:\", \"UNIVERSIDAD\");\n\tElimStr(texto);\n\tShowMessage(\"Resultado tras eliminar parejas vocal-consonante: \" + texto);\n}\n```\n\n\nPASO 4: COMPILAR Y EJECUTAR\n------------------------------------------------------------------------\n1. Guarda todo yendo a \"File\" > \"Save All\".\n2. Presiona la tecla F9 (o el triangulo verde) para ejecutar tu aplicacion.",
            ts: 1789819201000
        }
    ]
};
...
function loadConversations() {
    try {
        ...
        if (!list.some(c => c.id === RAD_STUDIO_CONV.id)) {
            list.unshift(RAD_STUDIO_CONV);
        }
        return list;
    }
    catch { return []; }
